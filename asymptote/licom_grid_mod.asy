...
// Last Change: 2013-04-02 14:26:35 CST
...
//    real hlen = (8-1)*md.dw;
    ...
    for (int i=0; i<3; ++i) {
        // left horizontal line of whole grid
        draw(pic, shift(i*dw,0)*line, lonpen);
        // left vertical line of half grid
        ...
        label(pic, "\tt "+(string)(i+1), pos, 0.3cm*S, lonpen);

        // bottom horizontal line of whole grid
        draw(pic, shift(0,i*dw)*rotate(-90)*hori_line, latpen);
        // bottom horizontal line of half grid
    ...
    for (int i=6; i<9; ++i) {
        // right vertical line of whole grid
        draw(pic, shift(i*dw,0)*line, lonpen);
        // right horizontal line of half grid
        draw(pic, shift((i-0.5)*dw,-0.5dw)*hline, dashed+lonpen);
        ...
        pair pos = (i*dw,0);
        label(pic, lab, pos, S, lonpen);
        label(pic, "\tt "+(string)(nlons - (8-i)), pos, 0.3cm*S, lonpen);

        // right vertical line of whole grid
        draw(pic, shift(0,i*dw)*rotate(-90)*hori_line, latpen);
        // up horizontal line of half grid
