...
% Last Change: 2014-08-20 21:06:44 BJT

% Set file names.

soda_dir = '/home/ou/archive/data/soda/climate/';
roms_dir = '/home/ou/archive/models/roms/roms_734/Apps/yellow_sea/in/';

fname = [soda_dir 'climate_1981-2000.nc'];
GRDname = [roms_dir 'yellow_sea_grid.nc'];
...
S.Tcline      = 5.0;       % S-coordinate surface/bottom stretching width
...
Zeta = nc_varget( fname, 'ssh' );  Zeta = Zeta';
Temp = nc_varget( fname, 'temp' ); Temp = permute(Temp, [3 2 1]);
Salt = nc_varget( fname, 'salt' ); Salt = permute(Salt, [3 2 1]);
Uvel = nc_varget( fname, 'u' );    Uvel = permute(Uvel, [3 2 1]);
Vvel = nc_varget( fname, 'v' );    Vvel = permute(Vvel, [3 2 1]);
...
%  Set global attributes.
  ...
  avalue='Hudson estuary, ~5km';
  [status]=nc_attadd(INIname,'title',avalue);

  avalue='SODA climate mean, 0.5 resolution';
