...
//--------------------------------------------------------------------
picture picTsf;
size(picTsf, slide.x*1.0/4);
pair posTsf = (slide.x*0.6/4, slide.y*1.5/4);
...
label(scale(1.5)*"\citep[P.102]{Asymptote2004}", posTsf, S);

// scientific style function
//--------------------------------------------------------------------
picture picSci;
real sizex = slide.x*1.0/4;
pair posSci = (slide.x*2.5/4, slide.y*0.6/3);

add(scale(1.2)*picSci.fit(), posSci, N);
label(scale(1.0)*graphic("scientific-style-func.eps", "width=23cm"), posSci, N);
label(scale(1.5)*"\citep[P.103]{Asymptote2004}", posSci, S);
