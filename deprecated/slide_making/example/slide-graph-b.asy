...
pair posMar = (slide.x*1.7/4, slide.y*1.4/3);
label(scale(1.4)*graphic("marker-errorbar.eps", "width=20cm"), posMar, N);
label(scale(1.5)*"marker and errorbars \citep[P.107]{Asymptote2004}", posMar, S);

pair posCus = (slide.x*1.7/4, slide.y*0.2/3);
label(scale(1.0)*graphic("custom-marker.eps", "width=10cm"), posCus, N);
label(scale(1.5)*"customized marker\citep[P.108]{Asymptote2004}", posCus, S);
